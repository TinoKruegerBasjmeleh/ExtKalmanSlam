...
def main() -> None:
    ...
    ax.set_title("EKF SLAM — Ideal (blue) vs Noisy (red) State Estimates", fontsize=13)
    ...
    for idx, i in enumerate(lm_ids):
        ...
        (m_noisy,) = ax.plot(
            [], [], "+", color=color, ms=9, mew=2.2, alpha=0.55, zorder=7
        )
        e_noisy = Ellipse(
            (0, 0),
            width=1,
        ...
        lm_legend_handles.append(mpatches.Patch(color=color, label=f"Landmark {i}"))
    ...
    # Legend
    legend_elems = [
        plt.Line2D([0], [0], color=IDEAL_COLOR, lw=1.5, label="Ideal trajectory"),
        mpatches.Patch(fc=IDEAL_COLOR, ec="navy", label="Ideal robot pose"),
        mpatches.Patch(fc="none", ec=IDEAL_COLOR, ls="--", label="Ideal uncertainty"),
        plt.Line2D([0], [0], color=NOISY_COLOR, lw=1.5, label="Noisy trajectory"),
        mpatches.Patch(fc=NOISY_COLOR, ec="darkred", label="Noisy robot pose"),
        mpatches.Patch(fc="none", ec=NOISY_COLOR, ls="--", label="Noisy uncertainty"),
    ] + lm_legend_handles
    ax.legend(handles=legend_elems, loc="upper right", fontsize=8, framealpha=0.85)

    # ── Update function ───────────────────────────────────────────────────────
    all_artists = (
        [ideal_trail, ideal_tri, ideal_ell, noisy_trail, noisy_tri, noisy_ell, time_txt]
        + ideal_lm_markers
        + ideal_lm_ellipses
        + ideal_lm_texts
        + noisy_lm_markers
        ...
        for idx, i in enumerate(lm_ids):
            ...
            if active:
                ...
                if texts is not None:
                    texts[idx].set_position((lx + robot_sz * 0.6, ly + robot_sz * 0.6))
    ...
    def update(frame_no: int):
        ...
        ideal_tri.set_xy(
            robot_vertices(row["robot_x"], row["robot_y"], row["robot_theta"], robot_sz)
        )
        ...
        noisy_tri.set_xy(
            robot_vertices(
                row["noisy_robot_x"],
                row["noisy_robot_y"],
                row["noisy_robot_theta"],
                robot_sz,
            )
        )
        noisy_ell.set_center((row["noisy_robot_x"], row["noisy_robot_y"]))
        noisy_ell.set_width(2.0 * N_SIGMA * np.sqrt(max(row["noisy_robot_std_x"], 0)))
        noisy_ell.set_height(2.0 * N_SIGMA * np.sqrt(max(row["noisy_robot_std_y"], 0)))

        # --- Landmarks ---
        update_lm(
            ideal_lm_markers, ideal_lm_ellipses, ideal_lm_texts, "", row, show_text=True
        )
        update_lm(
            noisy_lm_markers, noisy_lm_ellipses, None, "noisy_", row, show_text=False
        )
